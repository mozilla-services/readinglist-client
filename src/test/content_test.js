...
describe("ContentManager", function() {
  ...
  describe("#fetch()", function() {
    ...
    it("should fetch an article contents", function() {
      manager.client = returns(Promise.resolve({entity: {contents: "bar"}}));
    ...
    it("should save fetched contents in local storage", function(done) {
      ...
      manager.fetch(article).then(() => {
        ...
        sinon.assert.calledWithExactly(manager.save, article.id, "bar");
        done();
      });
    });

    it("should not save on fetch failure", function(done) {
      sandbox.stub(manager, "save");

      manager.client = returns(Promise.reject("boo"));

      manager.fetch(article).then(() => {
        sinon.assert.notCalled(manager.save);
        done();
      });
    });

    it("should return a null on fetch failure", function() {
      manager.client = returns(Promise.reject("boo"));

      return manager.fetch(article).should.become(null);
    });
  });
  ...
  describe("#load()", function() {
    ...
    it("should return a null when contents are unavailable", function() {
      manager.client = returns(Promise.reject("boo"));

      return manager.load(article).should.become(null);
    });
  });
